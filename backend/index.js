...
// Route pour récupérer tous les materials
...
// Route pour récupérer un material par son ID
app.get("/material/:id", (req, res) => {
  ...
  connection.query(
    ...
    (err, results) => {
      if (err) {
        console.error("Erreur lors de la récupération du material :", err);
        res.status(500).send("Erreur serveur");
        return;
      }
      if (results.length === 0) {
...
// Route pour créer un nouveau material
...
// Route pour mettre à jour un material
app.patch("/material/:id", upload, (req, res) => {
  const materialId = req.params.id;
  const material = req.body.material;
  ...
  connection.query("UPDATE material SET ? WHERE id = ?", [material, materialId], (err) => {
    if (err) {
      console.error("Erreur lors de la mise à jour du material :", err);
      res.status(500).send("Erreur serveur");
      return;
    }
    //NOK -> res.sendStatus(200);
...
// Route pour supprimer un material
app.delete("/material/:id", authenticateToken, (req, res) => {
  ...
  connection.query("DELETE FROM material WHERE id = ?", [materialId], (err) => {
    if (err) {
      console.error("Erreur lors de la suppression de la remarque :", err);
      res.status(500).send("Erreur serveur");
      return;
